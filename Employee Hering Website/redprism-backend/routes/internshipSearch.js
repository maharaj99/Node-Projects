...
// ===================================================
// ROUTER : 1 Get Active Location List ( GET method api : /api/internshipSearch/getLocationList )
// ===================================================
...
// ===================================================
// ROUTER : 2 Get Active Service Area List ( GET method api : /api/internshipSearch/getServiceAreaList )
// ===================================================
...
// ===================================================
// ROUTER : 3 Get Internship List ( POST method api : /api/internshipSearch/getInternshipList )
// ===================================================
router.post('/getInternshipList', verifyUser, [
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            ...
            // Manage Search Data 
            const searchData = [{ "_id": { $ne: '' } },];


            if (service_area_code) {
                searchData.push({ "service_area_code": new mongoose.Types.ObjectId(service_area_code) });
            }
            if (location_code) {
                searchData.push({ "locations.location_code": new mongoose.Types.ObjectId(location_code) });
            }

            // Fetch Internship List
            let intershipList = await internship_details.aggregate([
            ...
            if (intershipList) {
                return res.status(200).json({ status: 'success', mssg: 'Internship List Fetched Successfully', limit, intershipList });
            }
            else {
                return res.status(200).json({ status: 'error', mssg: 'Not Found', });
            }
